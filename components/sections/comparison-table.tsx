  ...
  return (
    <section id="comparison" className="scroll-mt-20 border-y border-blue-100 bg-blue-50/60 py-16 sm:py-20">
      <div className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8">
          ...
          <table className="w-full min-w-[1040px] border-collapse text-left text-sm">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                <th className="sticky left-0 z-10 min-w-36 bg-slate-50 px-5 py-4 font-bold text-slate-950">
                  对照维度
                </th>
                {methods.map((method) => (
                  <th key={method.id} className="min-w-44 px-5 py-4 font-bold text-slate-950">
                    {method.name}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {comparisonRows.map((row) => (
                <tr key={row.dimension} className="border-b border-slate-100 last:border-0">
                  <th className="sticky left-0 z-10 bg-white px-5 py-4 font-semibold text-slate-950">
                    {row.dimension}
                  </th>
                  {methods.map((method) => (
                    <td key={method.id} className="px-5 py-4 leading-6 text-slate-900">
        ...
        <p className="mt-3 text-sm text-slate-600">移动端可横向滑动查看完整矩阵。</p>
      </div>
    </section>
  );
}
